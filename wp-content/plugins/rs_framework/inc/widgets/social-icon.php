<?php /**
* Adds Finoptis Social Icon Widget widget
*/
class Finoptissocialiconwi_Widget extends WP_Widget {
	...
	*/
	function __construct() {
		parent::__construct(
			'finoptissocialiconwi_widget', // Base ID
			esc_html__( 'Finoptis Social Icon Widget', 'finoptis' ), // Name
			array( 'description' => esc_html__( 'finoptis social icon widget area', 'finoptis' ), ) // Args
		);
	}
	...
	*/
	public function field_generator( $instance ) {
		...
		foreach ( $this->widget_fields as $widget_field ) {
			$widget_value = ! empty( $instance[$widget_field['id']] ) ? $instance[$widget_field['id']] : esc_html__( isset($widget_field['default']), 'finoptis' );
			switch ( $widget_field['type'] ) {
				default:
					$output .= '<p>';
					$output .= '<label for="'.esc_attr( $this->get_field_id( $widget_field['id'] ) ).'">'.esc_attr( $widget_field['label'], 'finoptis' ).':</label> ';
	...
	public function form( $instance ) {
		$title = ! empty( $instance['title'] ) ? $instance['title'] : esc_html__( '', 'finoptis' );
		?>
		<p>
		<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_attr_e( 'Title:', 'finoptis' ); ?></label>
...
} // class Finoptissocialiconwi_Widget

// register Finoptis Social Icon Widget widget
function register_finoptissocialiconwi_widget() {
	register_widget( 'Finoptissocialiconwi_Widget' );
}
add_action( 'widgets_init', 'register_finoptissocialiconwi_widget' );